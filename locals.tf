locals {
    ...
    aci_leaf_access_port_policy_group_rows = {
        ...
        "${i.TENANT_NAME}:${i.ENDPOINT_MAKE}:${i.ENDPOINT_MODEL}:${i.ENDPOINT_OS}:${i.ENDPOINT_INTERFACE_TYPE}" => {
             ...
             LINK_LEVEL_POLICY_NAME            = i.LINK_LEVEL_POLICY_NAME 
        }
    } 
    ...
    aci_logical_interface_profile_iterations = csvdecode(file("./data/aci_logical_interface_profile.csv"))
    ...
    aci_bgp_route_summarization_iterations = csvdecode(file("./data/aci_bgp_route_summarization.csv"))

    aci_bgp_route_summarization_rows = {
        for i in local.aci_bgp_route_summarization_iterations : 
        "${i.TENANT_NAME}:${i.VRF_NAME}:${i.PEER_GROUP}" => {
             TENANT_NAME            = i.TENANT_NAME
             VRF_NAME               = i.VRF_NAME
             PEER_GROUP             = i.PEER_GROUP 
